...
data_modified %>% 
  ...
  count(decade)
```
Primjećujemo vrlo mali broj podataka u pedesetim i šezdesetim godinama, što može uzrokovati nekonzistentnost na grafičkom prikazu.


U nastavku istovremeno prikazujemo promjene prosječne danceability, energy i valence kroz godine, ograničavajući interval na 1970. godinu nadalje.
```{r}
ggplot(yearly %>% filter(relase_year >= 1970), aes(x = relase_year)) + 
  geom_line(aes(y = mean_d, color = "Danceability")) + 
  geom_line(aes(y = mean_e, color = "Energy")) + 
  geom_line(aes(y = mean_v, color = "Valence")) + 
  labs(
  ...
  theme_bw()
```
Podaci su stabilni, s vidljivim, iako sporim, ali svakako postojanim rastom mjera danceability i energy, dok je valence doživio nagli pad unazad od 2010. godine. Ovo možemo interpretirati kao da emocionalna pozitivnost novijih pjesama značajno opada.
